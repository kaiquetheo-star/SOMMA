/** Shared exercise classification — 5-Phase Clinical Matrix helpers. */
...
export function exerciseTextBlob(meta: ClinicalExerciseMeta | undefined): string {
  if (!meta) return '';
  return `${meta.primary_muscle ?? ''} ${meta.name ?? ''} ${meta.slug ?? ''}`.toLowerCase();
}

export function isCoreFinisher(meta: ClinicalExerciseMeta | undefined): boolean {
  if (!meta) return false;
  const primary = (meta.primary_muscle ?? '').toLowerCase();
  if (primary === 'abs' || primary === 'core' || primary === 'abdominals') return true;
  return /crunch|plank|russian twist|abdominal|oblique|dead bug|hollow hold|\bcore\b/.test(exerciseTextBlob(meta));
}

export function isIsolation(meta: ClinicalExerciseMeta | undefined): boolean {
  ...
  return /\bcurl\b|pushdown|push-down|lateral raise|front raise|\bfly\b|flye|kickback|pullover|extension/.test(
    exerciseTextBlob(meta),
  );
}

export function isHeavyCompound(meta: ClinicalExerciseMeta | undefined): boolean {
  if (!meta || isCoreFinisher(meta) || isIsolation(meta)) return false;
  const cns = meta.cns_fatigue_cost ?? 3;
  const name = exerciseTextBlob(meta);
...
export function isSecondaryCompound(meta: ClinicalExerciseMeta | undefined): boolean {
